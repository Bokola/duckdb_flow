...
# download specific extension for export

def load_aws_credentials(duckdb_con, profile: str):
    duckdb_con.sql(f"CALL load_adwa_credentials('{profile}');")


# write to s3_bucket
def write_to_s3_from_duckdb(duckdb_con, table: str, s3_path: str, timestamp_column: str):
...
def write_to_md_from_duckdb(
    ...
    # insert new data
    duckdb_con.sql(
        f"""INSERT INTO {remote_database}.main.{table}
            SELECT *
